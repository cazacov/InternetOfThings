...
void setup()
{
	//Serial.begin(9600);
	Wire.begin();
	Wire.getI2cHasBeenEnabled() ? wprintf(L"I2C is enabled") : wprintf(L"I2C is disabled");
	// Set Gyro settings
...
void loop()
{
	...
	swprintf(buf, 100, L"GX:%d\tGY:%d\tGZ:%d\tAX:%d\tAY:%d\tAZ:%d", gyro_x, gyro_y, gyro_z, accel_x, accel_y, accel_z);
