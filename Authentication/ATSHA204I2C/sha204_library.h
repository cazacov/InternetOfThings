...
#define SHA204_I2C_ADDRESS	((uint8_t) 0x64)
...
#define SET_DEVICE_PORT_TO_INPUT() pinMode(this->io_pin, INPUT)
#define SET_DEVICE_PORT_TO_OUTPUT() pinMode(this->io_pin, OUTPUT)
#else
#define IS_AVR
#define REGISTER_DATA_TYPE uint8_t*
#define PORT_DATA_TYPE uint8_t
#define SET_DEVICE_PORT_TO_INPUT() *this->device_port_DDR &= ~this->pinBitmask
#define SET_DEVICE_PORT_TO_OUTPUT() *this->device_port_DDR |= this->pinBitmask
#endif
...
#else
#define START_PULSE_TIME_OUT	(2500)	//! This value is decremented while waiting for the falling edge of a start pulse.
#define ZERO_PULSE_TIME_OUT		(250)	//! This value is decremented while waiting for the falling edge of a zero pulse.
#endif
/* swi_phys.h */
...
#define SHA204_WAKEUP_DELAY          (uint8_t) (3 * CPU_CLOCK_DEVIATION_POSITIVE + 0.5)	//! delay between Wakeup pulse and communication in ms
...
private:
	...
	uint32_t pinBitmask;
#endif
	volatile uint8_t *device_port_DDR;
	volatile REGISTER_DATA_TYPE device_port_OUT;
	volatile REGISTER_DATA_TYPE device_port_IN;
